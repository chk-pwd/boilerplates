...
resource "proxmox_vm_qemu" "k3s-master-3" {
  ...
  vmid = "54${count.index + 1}"

  # Proxmox target node
  target_node = var.node3
  ...
  ipconfig0 = "ip=172.16.16.7${count.index + 1}/24,gw=172.16.16.1"
...
resource "proxmox_vm_qemu" "k3s-agent-3" {
  ...
  ipconfig0 = "ip=172.16.16.8${count.index + 1}/24,gw=172.16.16.1"
